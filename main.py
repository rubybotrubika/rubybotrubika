...
Start = time.time()
sleep (0.5)
print ("ruby-bot-rubika")
Finish = time.time()
print(Start - Finish)

bot = Bot("AppName", auth="vhdhbkxjyioydbhfygqgofhusvpgbhof")
target = "g0BlLtM0d7a07ed0c1c36f352fb6a1cb"
...
def alert(guid,user,link=False):
	...
	haslink = ""
	if link : haslink = "گذاشتن لینک در کانال ممنوع است!🚫🚫\n\n"
				...
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						if hasAds(msg.get("text")) and not msg.get("author_object_guid") in admins :
							...
							try:
								bot.sendMessage(target, "ربات احساس داره به نظرت؟", message_id=msg.get("message_id"))
							except:
								print("err answer hay")
								
						elif msg.get("text").startswith("چه خبر؟") or msg.get("text").startswith("چطوری؟"):
							try:
								bot.sendMessage(target, "هیچ", message_id=msg.get("message_id"))
							except:
								print("err CheKhabar")
								
						elif msg.get("text").startswith("ربات") or msg.get("text").startswith("بات"):
							try:
								bot.sendMessage(target, "بلی", message_id=msg.get("message_id"))
							except:
								print("err bot answer")
								
						elif msg.get("text").startswith("😂") or msg.get("text").startswith("🤣"):
							try:
								bot.sendMessage(target, "جولمه پادارسان", message_id=msg.get("message_id"))
							except:
								print("err luagh")
